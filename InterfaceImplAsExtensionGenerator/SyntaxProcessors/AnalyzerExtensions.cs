...
static class SourceGeneratorExtensions
{
    ...
    public static string GenerateConstraintsFromSyntax(this TypeParameterSyntax typeParam)
    {
        var constraints = new List<string>();

        // 使用 Syntax 分析约束子句
        var constraintClauses = typeParam.Parent?
            .DescendantNodes()
            .OfType<TypeParameterConstraintClauseSyntax>()
            .Where(c => c.Name.Identifier.Text == typeParam.Identifier.Text)
            .FirstOrDefault();

        if (constraintClauses != null)
        {
            foreach (var constraint in constraintClauses.Constraints)
            {
                switch (constraint)
                {
                    case ClassOrStructConstraintSyntax classOrStruct:
                        constraints.Add(classOrStruct.ClassOrStructKeyword.Text);
                        break;
                    case TypeConstraintSyntax typeConstraint:
                        constraints.Add(typeConstraint.Type.ToString());
                        break;
                    case ConstructorConstraintSyntax constructor:
                        constraints.Add("new()");
                        break;
        ...
        return constraints.Count > 0
            ? $"where {typeParam.Identifier.Text} : {string.Join(", ", constraints)}"
            : string.Empty;
    }
